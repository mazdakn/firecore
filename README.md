...
At the top level, the `engine` package runs packets through one or more tables using `eval.Context` inputs and returns `eval.Result` values with verdicts plus rule traces. The surrounding domain packages provide the building blocks for that evaluation:
